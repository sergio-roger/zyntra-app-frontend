...
| Chat con el Equipo | `/agents/chat` | **Mockup visual, no funcional** | `AiAgentsChatPage.tsx`: réplica estática del diseño (roster de 6 agentes, hilo de chat con tarjetas de plan/entregables/métricas, panel derecho con tabs "Proceso del equipo"/"Herramientas"/"Tareas"/"Archivos"). Todo el contenido sale de `marketing/constants/chat-mock-data.ts` (tipos en `marketing/types/chat-mock.ts`) — no pega a ningún endpoint. Único estado real: el texto del composer (`useState` local) y la tab activa del panel derecho; el resto de los botones ("Nuevo objetivo", "...", "Ver plan completo", "Ver reporte completo", adjuntar/mencionar/sugerir) son decorativos. Componentes en `marketing/components/chat/` (uno por archivo: `ChatAgentRoster(Card)`, `ChatThread`, `ChatMessageBubble`, `ActionPlanCard`, `DeliverablesList`, `MetricsGrid`, `ChatComposer`, `TeamProcessPanel`, `ChatPanelTabBar`, `ProcessTimeline(Item)`, `Tools/Tasks/FilesTabContent`, `ChatStatusPill`). |
