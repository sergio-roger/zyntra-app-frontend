...
export const PlansPage: React.FC = () => {
  ...
  const renderValue = (val: boolean | string) => {
    ...
    return (
      <span className="font-semibold text-base-content/85 text-sm">{val}</span>
    );
  };
    ...
    <div className="container mx-auto px-4 py-8 max-w-6xl animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="text-center mb-12">
        ...
        <p className="mt-4 text-lg text-base-content/60 max-w-2xl mx-auto">
          Elige el plan ideal para escalar tu negocio y potenciar tus ventas con
            ...
            >
              ...
              <div className="p-8 pb-0">
                <div className="flex items-center justify-between">
                  <span
                    className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${plan.badgeBg}`}
                  >
                    {plan.badgeText}
                  </span>
                </div>

                <h3 className="mt-4 text-2xl font-bold text-base-content">
                  {plan.name}
                </h3>
          ...
          <table className="table table-zebra w-full text-left">
            <thead>
              <tr className="border-b border-base-300 text-base-content/70">
                <th className="py-4 text-sm font-bold w-1/3">
                  Características
                </th>
                <th className="py-4 text-sm font-bold text-center">
                  BrandStart
                </th>
                <th className="py-4 text-sm font-bold text-center">
                  Impulse Pro
              ...
              {comparisons.map((row, idx) => {
                if (row.isHeader) {
                  return (
                    <tr
                      key={idx}
                      className="bg-base-300/40 border-b border-base-300"
                    >
                      <td
                        colSpan={4}
                        className="py-3 px-4 text-xs font-extrabold uppercase tracking-wider text-primary"
                      >
                        {row.category}
                      </td>
                    </tr>
                ...
                return (
                  <tr
                    key={idx}
                    className="border-b border-base-300/50 hover:bg-base-300/20 transition-colors"
                  >
                    <td className="py-3.5 px-4 text-sm text-base-content/80 font-medium">
                      {row.name}
                    </td>
                    <td className="py-3.5 text-center">
                      {renderValue(row.brand!)}
                    </td>
                    <td className="py-3.5 text-center">
                      {renderValue(row.impulse!)}
                    </td>
                    <td className="py-3.5 text-center">
                      {renderValue(row.core!)}
                    </td>
                  </tr>
                );
              })}
