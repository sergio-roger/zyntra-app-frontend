...
export const CardWrapper: React.FC<CardWrapperProps> = ({
  ...
  const baseClasses = 'relative flex flex-col justify-between rounded-lg border transition-all duration-300';

  const variantClasses = {
    default: 'bg-slate-900/50 border-white/5 shadow-xl text-white',
    highlighted: 'border-primary ring-2 ring-primary/20 scale-[1.01] shadow-2xl bg-slate-900/80 text-white',
    interactive: 'bg-base-200 border-base-300 text-base-content',
  };

  const hoverClasses = hoverable && variant !== 'highlighted'
