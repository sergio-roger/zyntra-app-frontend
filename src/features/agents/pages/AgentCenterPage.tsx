...
export const AgentCenterPage: React.FC = () => {
  ...
  const getIcon = (type: AgentTaskType) => {
    switch (type) {
      case AgentTaskType.CONTENT: return <MessageSquareText size={20} />;
      case AgentTaskType.SOCIAL: return <Share2 size={20} />;
      case AgentTaskType.CHATBOT: return <Bot size={20} />;
      case AgentTaskType.CRM_ANALYSIS: return <FileSearch size={20} />;
      case AgentTaskType.REPORT: return <BarChart size={20} />;
    }
  };
  ...
  const getStatusBadge = (status: AgentTaskStatus) => {
    switch (status) {
      case AgentTaskStatus.PENDING: 
        return <div className="badge badge-ghost gap-2"><Clock size={12}/> Pendiente</div>;
      case AgentTaskStatus.RUNNING: 
        return <div className="badge badge-info gap-2 animate-pulse"><Loader2 size={12} className="animate-spin"/> Trabajando...</div>;
      case AgentTaskStatus.COMPLETED: 
        return <div className="badge badge-success gap-2"><CheckCircle2 size={12}/> Completado</div>;
      case AgentTaskStatus.FAILED: 
        return <div className="badge badge-error gap-2"><XCircle size={12}/> Error</div>;
    }
  };
        ...
        <div className="grid gap-4">
          {filteredTasks.length === 0 ? (
            <div className="card bg-base-200 border-2 border-dashed border-base-300 p-12 text-center">
              ...
              <h3 className="text-xl font-semibold">No hay tareas de {context.title.toLowerCase()}</h3>
              <p className="text-base-content/60 mt-2">Comienza lanzando tu primera solicitud a los agentes.</p>
              <button 
                onClick={() => setIsModalOpen(true)}
            ...
            filteredTasks.map((task) => (
              <div key={task._id} className="card bg-base-100 shadow-sm border border-base-200 hover:shadow-md transition-shadow">
      ...
      {isModalOpen && (
        <div className="modal modal-open">
          <div className="modal-box max-w-md">
            <h3 className="font-bold text-xl mb-6">Nuevo Agente: {context.title}</h3>
            <form onSubmit={handleCreate} className="space-y-4">
              <div className="form-control">
                <label className="label"><span className="label-text">Tópico o Título</span></label>
                <input 
                  type="text" 
              ...
              <div className="form-control">
                <label className="label"><span className="label-text">Instrucciones Extra (Opcional)</span></label>
                <textarea 
              ...
              <div className="modal-action">
                ...
                <button type="submit" disabled={isCreating} className="btn btn-primary">
                  {isCreating && <Loader2 className="animate-spin mr-2" size={18} />}
                  Lanzar Crew
                </button>
              </div>
            </form>
      ...
      {selectedTask && (
        <div className="modal modal-open">
          <div className="modal-box max-w-4xl max-h-[80vh]">
            ...
            <div className="modal-action">
              <button onClick={() => setSelectedTask(null)} className="btn btn-primary">Entendido</button>
            </div>
          </div>
        </div>
      )}
