...
const COLORS = [
  '#4F46E5', '#10B981', '#F59E0B', '#EF4444', '#EC4899', '#8B5CF6', '#06B6D4', '#F97316'
];

export const TeamFormSidebar: React.FC<TeamFormSidebarProps> = ({ open, team, onClose }) => {
        ...
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between p-6 border-b border-white/5 bg-slate-900/50 backdrop-blur-md">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">Organiza a tus colaboradores por áreas o funciones</p>
            </div>
          ...
          <form id="team-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-6">
            <Input
              ...
              <div className="space-y-2 max-h-[200px] overflow-y-auto pr-2 custom-scrollbar">
                ...
                {users.map(u => (
                  ...
                  >
                    ...
                    {formData.member_ids.includes(u.id) && <Check size={16} className="text-indigo-400" />}
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="p-6 border-t border-white/5 bg-slate-900/50 backdrop-blur-md">
            <div className="flex gap-3">
              ...
              <button form="team-form" type="submit" disabled={isSaving || !formData.name} className="flex-[2] px-4 py-3 rounded-xl bg-indigo-600 text-white text-sm font-bold hover:bg-indigo-500 transition-all shadow-lg shadow-indigo-500/20 flex items-center justify-center gap-2 disabled:opacity-50">
