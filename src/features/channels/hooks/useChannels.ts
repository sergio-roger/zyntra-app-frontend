...
import type { CreateChannelPayload, UpdateChannelPayload } from '../types/channels.types';
...
export const useCreateChannel = () => {
  ...
  return useMutation({
    ...
    onSuccess: () => qc.invalidateQueries({ queryKey: ['channels', businessId] }),
  });
};
...
export const useAssignAgent = (channelId: string) => {
  ...
  const businessId = useBusinessId();
  return useMutation({
    mutationFn: (agentId: string) => channelsApi.assignAgent(businessId, channelId, agentId),
    onSuccess: () => {
