...
export const ContactCustomFieldsSidebar: React.FC<ContactCustomFieldsSidebarProps> = ({
  open,
  contact,
  onClose,
}) => {
  ...
  const handleSave = async () => {
    if (!contact) return;
    await updateMutation.mutateAsync({ id: contact.id, input: { customFields: values } });
    onClose();
  };
      ...
      >
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between p-6 border-b border-white/5">
            <div>
              <h3 className="text-xl font-bold text-white">Campos personalizados</h3>
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {activeFields.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-16 opacity-30 border-2 border-dashed border-white/10 rounded-2xl">
                ...
                <p className="text-sm text-slate-500">No hay campos personalizados activos</p>
              </div>
            ) : (
