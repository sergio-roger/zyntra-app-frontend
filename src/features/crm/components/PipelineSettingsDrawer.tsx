...
export const PipelineSettingsDrawer: React.FC<PipelineSettingsDrawerProps> = ({
  ...
  const [deletingStage, setDeletingStage] = useState<EditableStage | null>(
    null,
  );
  ...
  const handleSaveStage = async (stage: EditableStage) => {
    ...
    try {
      ...
      setStages((prev) =>
        prev.map((s) => (s.id === stage.id ? { ...s, _dirty: false } : s)),
      );
    } finally {
      setSavingId(null);
    }
  ...
  const handleAddStage = async () => {
    const nextColor =
      PIPELINE_STAGE_COLORS[stages.length % PIPELINE_STAGE_COLORS.length];
    try {
          ...
          {activeTab === "etapas" && (
            <div className="p-6 space-y-3">
              ...
              {stages.map((stage) => (
                ...
                >
                  <div
                    className="h-0.5 w-full rounded-full -mt-1 mb-2"
                  ...
                  <div className="flex items-center gap-3 pl-5">
                    ...
                    >
                      {(Object.keys(STAGE_TYPE_LABELS) as StageType[]).map(
                        (t) => (
              ...
              {stages.length === 0 && (
                <div className="flex flex-col items-center justify-center py-16 text-center opacity-50">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    Agrega una etapa para comenzar.
                  </p>
                </div>
              )}
            </div>
